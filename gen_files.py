...
def generate_multi_turn_files(args):
    ...
    for idx, file_name in enumerate(file_list):
        ...
        try:
            utterances = [data['utterance'] for data in json_data]
        except:
            utterances = [data['utterence'] for data in json_data]  #json 파일 중 utterance가 utterence로 오타난 경우가 있어 예외 처리
        sentences = []
        for i in range(0, len(utterances)-1, 2):
            sentences.append({'q': utterances[i], 'a': utterances[i+1]})
            ids.append('id_'+str(idx))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_path", type=str, default="/Users/dannykm/Downloads/[솔트룩스] 대화 데이터 샘플 v1.2.0")
